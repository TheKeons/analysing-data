...
class Placeholder:
    def __init__(self, file, file2):
        ...
        self.average_growth_rate_30_2 = (self.k[-1] - self.k[len(self.k) - 30]) / \
                                        (self.t[-1] - self.t[len(self.k) - 30])
        self.average_growth_rate_15_2 = (self.k[-1] - self.k[len(self.k) - 15]) / \
                                        (self.t[-1] - self.t[len(self.k) - 15])
        ...
        self.pred_30 = [point_slope_form(self.average_growth_rate_30, self.x1, self.y1)(i) for i in self.x_prediction]
        self.pred_15 = [point_slope_form(self.average_growth_rate_15, self.x1, self.y1)(i) for i in self.x_prediction]
        self.pred_5 = [point_slope_form(self.average_growth_rate_5, self.x1, self.y1)(i) for i in self.x_prediction]

        self.pred_30_2 = [point_slope_form(self.average_growth_rate_30_2,
                                           self.t1, self.k1)(i) for i in self.x_prediction]
        self.pred_15_2 = [point_slope_form(self.average_growth_rate_15_2,
                                           self.t1, self.k1)(i) for i in self.x_prediction]
        self.pred_5_2 = [point_slope_form(self.average_growth_rate_5_2, self.t1, 
                                          self.k1)(i) for i in self.x_prediction]

    # Chooses which type of graph to make
    def plot(self, graph_type, goal, populations, names: tuple):
        match graph_type:
            case 1:
    ...
    def plot_derivative(self, goal, populations, names):
        match populations:
            case 'uno':
                plt.plot(self.x[1:], [i * 1000000 for i in der(self.x, self.y)])
            
            case 'dos':
                plt.plot(self.t[1:], [i * 1000000 for i in der(self.t, self.k)])
            
            case 'both':
                plt.plot(self.x[1:], [i * 1000000 for i in der(self.x, self.y)], label=names[0])
                plt.plot(self.t[1:], [i * 1000000 for i in der(self.t, self.k)], label=names[1])
...
def point_slope_form(growth_rate, x1, y1):
